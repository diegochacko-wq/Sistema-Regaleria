...
export default function SelectorNegocio() {
  ...
  useEffect(() => {
    // Detectar si Supabase procesa el link de recuperación por hash en la URL
  ...
  const handleGuardarNuevaPassword = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setTimeout(() => {
        ...
        window.location.href = '/'; // Limpia el hash y recarga limpio
      }, 2000);
    }
  };

  // SI EL USUARIO HIZO CLIC EN EL MAIL DE RECUPERO, MOSTRAMOS ESTO PRIMERO
      ...
      <div className="min-h-screen bg-[#111111] flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-[#1a1a1a] border border-[#262626] rounded-2xl shadow-2xl p-8 text-white">
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold tracking-tight text-amber-500 mb-2">⚡ TONEXOR</h1>
            <p className="text-gray-400 text-sm">Establecé tu nueva contraseña de acceso.</p>
          </div>

          <div className="bg-white rounded-xl p-6 text-black shadow-inner">
            <h2 className="text-lg font-bold text-gray-800 mb-4 text-center">Nueva Contraseña</h2>
